...
struct RecipeTraits {
    ...
    static std::string Url(uint32_t id);                 // /v2/recipes/:id
    static bool Parse(const std::vector<char>&, Meta&);  // recipe body -> transient fields
    static std::string FallbackUrl(uint32_t) { return ""; }
    static bool ParseFallback(const std::vector<char>&, Meta&) { return false; }
    static bool ResolveDeps(Meta&, const HttpFetch&);    // Task 3
